...
*/
//-----------------------------------------------------------------------------------
Animation::Animation( Sint32 KeyMax )
{
	// メモリ確保
	m_KeyMax = KeyMax;
	...
	// 後にリンクリストを接続
	for ( Sint32 i = 0; i < KeyMax - 1; i++ )
	{
		m_pKeys[i].pNext = &m_pKeys[i+1];
	}

	// 前にリンクリストを接続
	for ( Sint32 i = KeyMax - 1; i >= 1; i-- )
	{
...
*/
//-----------------------------------------------------------------------------------
SKeyFrame *Animation::GetKeyData( SKeyFrame *pKeys, Float fTime, Sint32 KeyMax )
{
	Sint32 Left	= 0;
...
*/
//-----------------------------------------------------------------------------------
Bool Animation::SetKey( Sint32 KeyNo, Float fTime, Float fParam, Bool IsIn, Bool IsOut, Float fIn, Float fOut )
{
	if ( KeyNo < m_KeyMax )
	{
		// キー設定
...
*/
//-----------------------------------------------------------------------------------
Float Animation::InComing( SKeyFrame *pKey0, SKeyFrame *pKey1 )
{
...
*/
//-----------------------------------------------------------------------------------
Float Animation::OutGoing( SKeyFrame *pKey0, SKeyFrame *pKey1 )
{
	Float fParam = pKey1->fParam - pKey0->fParam;
...
*/
//-----------------------------------------------------------------------------------
Float Animation::GetParameter( Float fTime, eAnimationType Type, Float fDefault )
{
	//--------------------------------------------------
	// 時間はクランプ処理
	//--------------------------------------------------
