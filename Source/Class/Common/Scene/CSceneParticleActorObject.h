...
namespace Scene
{
	class CParticleActorObject : public CActorObject
	{
	protected:
		Bool m_IsSoftBillboardEnable;

	public:
		CParticleActorObject( CSceneManagerBase *pScene );
		~CParticleActorObject();

		virtual void UpdateMaterial( Renderer::CRender *pRender );
		virtual void SetSoftBillboardEnable( Bool IsSoftBillboard );
