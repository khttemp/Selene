...
namespace Renderer
{
	class CTargetTexture : public CBaseTexture, public CTextureInterface
	{
	protected:
		IDirect3DTexture9	*m_pTexture;
		IDirect3DTexture9	*m_pBaseTexture;
		Bool				m_IsFilterEnable;

	protected:
		virtual Bool DownloadToSystem( void );
		virtual Bool UploadToVram( void );
	...
	public:
		...
		virtual eTextureType GetType( void ) { return TEXTURE_TYPE_TARGET; }
		virtual eDepthBufferType GetDepthBufferType( void ) { return DEPTH_BUFFER_SURFACE; }

	public:
		CTargetTexture( CRenderObjectMgr *pMgr );
		...
		virtual Bool CreateRenderTarget( Sint32 Width, Sint32 Height, eSurfaceFormat Format );

		virtual Math::Point2DI GetSize( void );
		virtual Math::Point2DI GetOriginalSize( void );
		virtual Bool BackupTargetBuffer( void );
		virtual Bool IsFilterEnable( void );
	};
}
}
